...
class Calculator(QWidget):
    ...
    def buttonClicked(self):
        ...
        elif self.key in constantList :
            self.display.setText(self.display.text() + calcFunctions.constant(self))


        elif self.key == 'factorial (!)':
            n = self.display.text()
            try:
                value = str(fact(int(n)))
            except:
                value = 'Error!'

            self.display.setText(str(value))

        elif self.key in functionList:
            value = calcFunctions.function(self)
            self.display.setText(str(value))

        else:
            self.display.setText(self.display.text() + self.key)
